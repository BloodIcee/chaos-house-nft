...
namespace FIMSpace.Generating
{
    public partial class FieldSetup : ScriptableObject
    {
        ...
        internal FieldSetup GenerateRuntimeInstance()
        {
            ...
            newSetup.RootPack = ModificatorsPack.Instantiate(RootPack);

            for (int i = 0; i < newSetup.ModificatorPacks.Count; i++)
            {
                if (newSetup.ModificatorPacks[i] == newSetup.RootPack) continue;
                newSetup.ModificatorPacks[i] = ModificatorsPack.Instantiate(newSetup.ModificatorPacks[i]);
            }
