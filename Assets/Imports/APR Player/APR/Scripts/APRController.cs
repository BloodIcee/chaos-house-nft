...
public class APRController : MonoBehaviour
{
    ...
    void GroundCheck()
    {
        ...
        else if (!Physics.Raycast(ray, out hit, balanceHeight * 10, 1 << LayerMask.NameToLayer("Ground")))
        {
            if (balanced)
            {
                balanced = false;
    ...
    public void PlayerMovement(Vector3 dirrection, float movementSpedd)
    {
        if (movementSpedd != 0 && balanced)
        {
    ...
    public void PlayerMovement(bool state, Vector2 swipe, float forwardSpeed)
    {
        //Move in camera direction
        if (state)
        {
            Direction = APR_Parts[0].transform.rotation * new Vector3(swipe.x, 0.0f, forwardSpeed);
            Direction.y = 0f;
            ...
            if (swipe.x != 0 || swipe.y != 0 && balanced)
            {
                if (!WalkForward && !moveAxisUsed)
                {
                    WalkForward = true;
    ...
    public void PlayerMovement(float speed)
    {
        ...
        if (speed != 0)
        {
            var v3 = APR_Parts[0].GetComponent<Rigidbody>().transform.forward * (speed * speed);
            v3.y = APR_Parts[0].GetComponent<Rigidbody>().velocity.y;
            APR_Parts[0].GetComponent<Rigidbody>().velocity = v3;
        }
    ...
    //---Player Rotation---//
    public void PlayerRotation(Vector3 dirrection, float rotationSpeed)
    {
        if (dirrection == Vector3.zero) return;
        var lookPos = dirrection;
        lookPos.y = 0;
        var configJoint = APR_Parts[0].GetComponent<ConfigurableJoint>();
        var rotation = Quaternion.LookRotation(-lookPos);
        configJoint.targetRotation = Quaternion.Slerp(configJoint.targetRotation, Quaternion.Inverse(rotation), Time.deltaTime * rotationSpeed);

    }
    public void PlayerRotation(Vector3 dirrection)
    {
        var lookPos = dirrection;
        lookPos.y = 0;
        var configJoint = APR_Parts[0].GetComponent<ConfigurableJoint>();
        var rotation = Quaternion.LookRotation(lookPos);
        configJoint.targetRotation = Quaternion.Inverse(rotation);
        //configJoint.targetRotation = rotation;
    }
    ...
    public void JumpToPoint(Vector3 point, float forceJump)
    {
        var v3 = (point + APR_Parts[0].GetComponent<Rigidbody>().transform.up * 2f) * forceJump;
        v3.x = APR_Parts[0].GetComponent<Rigidbody>().velocity.x;
        v3.z = APR_Parts[0].GetComponent<Rigidbody>().velocity.z;
        APR_Parts[0].GetComponent<Rigidbody>().velocity = v3;
    ...
    public void PlayerPunch(EDirection direction)
    {

        //punch right
        if (!punchingRight && direction == EDirection.Right)
        {
            punchingRight = true;
        ...
        //punch left
        if (!punchingLeft && direction == EDirection.Left)
        {
            punchingLeft = true;
    ...
    public void PunchRight(Vector3 target)
    {
        var direction = target - RightHand.position;
        RightHand.AddForce(direction * punchForce, ForceMode.Impulse);
        //APR_Parts[1].GetComponent<Rigidbody>().AddForce(APR_Parts[0].transform.forward * punchForce, ForceMode.Impulse);
    }
    public void PunchLeft(Vector3 target)
    {
        var direction = target - RightHand.position;
        LeftHand.AddForce(direction * punchForce, ForceMode.Impulse);
